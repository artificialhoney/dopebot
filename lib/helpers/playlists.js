...
function lookupPlaylist (argv, playlist, offset, callback) {
  ...
  argv.db.spotify.getPlaylistTracks(playlist, { offset: offset, limit: 100, fields: 'items,total' })
    .then(function (data) {
      // when a playlist is played, replace the argv.db.search.results.tracks to selectively play a track in the playlist
      // as ordered in the the tracklist through command "tracks"
      let hit = 0
      ...
      for (let i = 0; i < data.body.items.length; i++) {
        ...
        // add tracks in  argv.db.search.results.tracks to be able to play a track from the playlist on comman play [id]
        argv.db.search.results.tracks.items[i] = data.body.items[i].track
        argv.db.search.results.tracks.items[i].hit = hit
        hit++
      }
