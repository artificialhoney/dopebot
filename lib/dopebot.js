...
DopeBot.prototype.updateHistoryState = function() {
  ...
  for (var user in this.history) {
    for (var tlid in this.history[user].tracks) {
      if (this.state.tlTrack.tlid === parseInt(tlid)) {
        for(var u in this.info.users) {
          if (this.info.users[u].id === user) {
            this.state.history = {
              user: this.info.users[u].name,
              timestamp: moment(this.history[user].tracks[tlid].timestamp).fromNow()
            }
            return;
          } 
        }
