...
module.exports = [
      ...
      .then(function(data) {
        if (data && data.albums) {
          for(var i = 0; i < data.albums.length; i++) {
            Object.assign(lastSearchResults.albums.items[i], data.albums[i]);
          }
        }
  ...
  {
    regEx: /queue (spotify:[\w:]+[a-zA-Z0-9]{22})( \d+)?/,
    help: 'Enqueues a Spotify URI at an optional position in the tracklist',
    execute: function(message, db) {
      var matches = message.text.match(this.regEx);
      var uri = matches[1];
