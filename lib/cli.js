...
module.exports = function () {
  ...
  spotifyApi.authorizationCodeGrant(argv.code)
    .then(function(data) {
      logger.debug('Spotify: The token expires in ' + data.body['expires_in'])
      logger.debug('Spotify: The access token is ' + data.body['access_token'])
      logger.debug('Spotify: The refresh token is ' + data.body['refresh_token'])
  ...
  spotifyApi.refreshAccessToken()
    .then(function(data) {
      logger.debug('Spotify: The access token has been refreshed!')

      // Save the access token so that it's used in future calls
      spotifyApi.setAccessToken(data.body['access_token'])
    }, function(err) {
      logger.error('Spotify: Could not refresh access token', err.message)
    })
