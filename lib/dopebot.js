...
DopeBot.prototype.broadcast = function(message) {
  for(var i in this.channels) {
    this.post(this.channels[i].id, message);
  }
}
...
DopeBot.prototype.getState = function() {
  var self = this;
  q.all([
  ...
  .done(function(result) {
    ...
    self.state.tlTrack = result['1'];
    self.state.volume = result['2'];
    self.state.consume = result['3'];
    self.state.random = result['4'];
    self.state.repeat = result['5'];
    self.state.single = result['6'];
  });
}
