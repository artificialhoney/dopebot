...
DopeBot.prototype.post = function(channel, message) {
  this.slack.postMessage(channel, entities.decode(message), this.params);
}
...
DopeBot.prototype._init = function() {
  ...
  this.slack.on('start', function () {
    ...
    self.command({regEx: /state/, help: 'Gets the current playback state', execute: function(message) {
  ...
  this.mopidy.on('event:trackPlaybackStarted', function (event) {
    ...
    if (self.settings.history === true) {
      for (var user in self.history) {
        for (var tlid in self.history[user].tracks) {
          if (event.tl_track.tlid === parseInt(tlid)) {
            var name
            for(var u in self.info.users) {
              if (self.info.users[u].id === user) {
                self.state.history = {
                  user: self.info.users[u].name,
                  timestamp: moment(self.history[user].tracks[tlid].timestamp).fromNow()
                }
  ...
  this.mopidy.on('event:playbackStateChanged', function(event) {
    self.logger.info('Plaback state changed');
