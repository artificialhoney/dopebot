...
DopeBot.prototype.post = function (message, channel) {
  channel = channel || this.channel.id
...
DopeBot.prototype._init = function () {
  ...
  this.yargs.commandDir('commands', {
    visit: function (commandObject) {
      const handler = commandObject.handler
      commandObject.handler = function (argv) {
        handler.call(self, argv)
      }
  ...
  this.mopidy.on('event:trackPlaybackStarted', function (event) {
    ...
    self.post(self.templates.state({state: self.state}))
  })
  ...
  this.slack.rtm.on(RTM_EVENTS.MESSAGE, function (message) {
    ...
    var argv = self.yargs.parse(message.text, {
      message: message
    }, function(exitError, parsed, output) {
      if (parsed.help) {
        self.post(output.replace(parsed['$0'], ''))
      }
    })
})
}
