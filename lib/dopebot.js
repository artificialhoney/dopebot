...
DopeBot.prototype._init = function() {
  ...
  self.slack.on('start', function () {
    self.bot = this.self;
    self.channels = [];
    self.baseRegEx = self.settings.dialog === true ? new RegExp('<@' + self.bot.id + '>[ ]+') : /^/;
    self.logger.info('Connected to Slack. I am %s', self.bot.id);

    self.emitter.on(/.*/, function(message) {
      // Listen to all events, including 'error', 'newListener',
      // and 'removeListener'
      if (message && message.channel && self.channels.indexOf(message.channel) === -1) {
    ...
    self.emitter.on(searchRegEx, function(message) {
      ...
      .then(function(data) {
        self.slack.postMessageToChannel(message.channel, searchTemplate({response: self.lastSearchResults = data.body}));
      }, function(err) {
    ...
    self.emitter.on(playRegEx, function(message) {
      ...
      if (!self.lastSearchResults || !self.lastSearchResults[type] || !self.lastSearchResults[type].items[id]) {
        self.slack.postMessageToChannel(message.channel, playTemplate()) ;
        return;
      }
      ...
      .done(function(tracks) {
        ...
        self.slack.postMessageToChannel(message.channel, playTemplate({item: item}));
      });
    ...
    self.emitter.on(playSpotifyEx, function(message) {
      ...
      .done(function(tracks) {
        ...
        self.slack.postMessageToChannel(message.channel, playSpotifyTemplate({uri: uri}));
      });
    ...
    self.emitter.on(currentRegEx, function(message) {
      ...
      .done(function(track) {
        self.slack.postMessageToChannel(message.channel, currentTemplate({track: track}));
      });
    });
    ...
    self.emitter.on(tracksRegEx, function(message) {
      ...
      .done(function(tracks) {
        self.slack.postMessageToChannel(message.channel, tracksTemplate({tracks: tracks}));
      });
    });
    ...
    self.emitter.on(volumeRegEx, function(message) {
      ...
      if(matches[1] !== undefined) {
        ...
        .done(function(volume) {
          self.slack.postMessageToChannel(message.channel, volumeTemplate({volume: volume}));
        });
      }
    });
    ...
    self.emitter.on(stateRegEx, function(message) {
      ...
      .done(function(state) {
        self.slack.postMessageToChannel(message.channel, stateTemplate({state: state}));
      });
    });
    ...
    self.emitter.on(helpRegEx, function(message) {
      self.slack.postMessageToChannel(message.channel, helpTemplate({
        commands: {
          'search <types...> <query>': {
    ...
    self.mopidy.on('event:trackPlaybackStarted', function (event) {
      self.logger.info('Track playback started');
      for(var i in self.channels) {
        self.slack.postMessageToChannel(self.channels[i], currentTemplate({track: event.tl_track.track}));
      }
    });
  });
